...
class AuditRepo:
    ...
    async def find_sign_and_send_seq(self, tx_id: str) -> int | None:
        """Return the seq of the approved sign-and-send audit row for *tx_id*, or None.

        Uses a substring match on the compact outcome JSON generated by a6
        (_canonical_json({"tx_id":..., "hash":..., "nonce":...})). tx_id is a
        UUIDv7 with no JSON-special characters, so the substring is reliable.

        Returns None when:
          - No matching row (pre-Phase-7 entities, row pruned).
          - tx_id is unknown.
        """
        result = await self._session.execute(
            select(audit_log.c.seq)
            .where(
                audit_log.c.action == "sign-and-send",
