...
## D25. Grants are keyed by `capability_id` — ADR-0001 dialect adoption (v1.1.0a98, constitutional amendment)

**Status:** Accepted — operator-authorized 2026-06-10, ratified under flaresystems **ADR-0003** (interface-cohesion; consumer #2 imminent). A constitutional amendment to the grant doctrine, bundled with the current-state-doc reconciliation it requires (Core #18). Self-ratifying per the constitutional-amendment ship-type (operator-authorized, never a Reviewer self-grant). Supersedes the ADR-0002 deferral (D24's "deferred until consumer #2").

**Context.** ADR-0001 §3 makes `capability_id` (`<consumer>/<network>/<role>`) the spine of the bundle/grant lifecycle: "spec requests → fwd grants/mints for it → … → doctor reconciles on it." Through a97 fwd grants were keyed only by `(name, policy_path)` (`caller_create.py`, `domain/policy.py::CallerBinding{policy_path}`) and `capability_id` did not exist in fwd. **This ship threads `capability_id` through the grant so the spine becomes a real join, and supersedes the a92-era "grants keyed by caller-name / policy_path only" design.** ADR-0003 frames the program as interface-cohesion: **clif = the reference consumer**; **`fwd-client` = the shared primitive (unchanged by this ship)**.

**Decision (the locked design — Unit-3 narrowing).** `capability_id` is **POLICY-SCHEMA-ONLY** — added as an optional, pattern-validated field on `domain.CallerBinding` (and echoed through the create request/response + the hash-chained audit row), with **NO Alembic / custody-DB migration** (it is not a `callers`-table column). The loader (`check_consistency`) enforces `capability_id` uniqueness across `CallerBinding`s (the join's integrity). A new operator-gated `fwdctl capability grant` ingests a consumer's `<consumer> spec --json` (paste-in via stdin/`--spec-file`; **no fwd→clif call** — zero-egress intact), **re-renders the ADR-§4 custody diff itself** (never trusting the consumer's rendered diff), and — default-deny, only on explicit `--approve` (Core #15) — mints each caller via the audited `POST /v1/admin/callers` keyed by `capability_id`. **`generate_policy` stays UNCHANGED** — the consumer-supplied spec supplies only (a) the `capability_id` and (b) the re-rendered diff for operator judgment; it never seeds policy *content* (Inv #6 — the consumer requests, the human-gated fwd side instantiates from fwd's own template). Back-compat: a name-only `callers create` (no `capability_id`) still validates. The signing path (`policy_engine`, sealed master, nonce manager, the sign endpoints) is **UNTOUCHED**.

**Deviation #1 — bounded realization (recorded honestly, Core #18).** The spec sketched a single "mint + bind + write policy block + reload" custody action. Phase-0 against live code found that **unrealizable as one in-container command**: policy reload requires a container recreate (a host/compose op outside the in-container CLI), and the spec JSON carries the wallet *name* but not the fwd caller-name/policy_path (the mint derives them via the canonical clif/onboard convention). So **`fwdctl capability grant` mints + re-renders the diff but does NOT write `policy.yaml` and does NOT emit a handoff bundle** — the operator authors `policy.yaml` (the `capability_id` lives on the `CallerBinding` there + the audit row), consistent with fwd's print-only / operator-authors-config custody posture (Core #6 caller tokens are bearer creds never persisted/emitted by fwd; Core #12 operator-controlled private config). Whether fwd's app should ever *write* `policy.yaml`, and the handoff-bundle / `--clif-env-dir` retirement, are **deferred to Unit 4**. `callers list` surfacing `capability_id` (a DB↔policy join) is the **Unit 5** read surface.

**Bounded surface.** Code (`src/` + `install/`, implementer, accepted — the one Part-A correction was a Core-#19 real-`session_scope`-rollback test, not a code change): `domain/policy.py`, `app/caller_create.py`, `app/capability_grant.py` (new), `api/callers.py`, `cli/callers.py`, `cli/capability.py` (new), `cli/main.py`, `infra/policy_loader.py`, `install/onboard` (the 3 live clif `caller_ensure` mints now carry `clif/<net>/{claim,fsp-sign,fsp-submit}`; `env_write`/`--clif-env-dir` untouched). Doctrine (Reviewer): this D-record, `CLAUDE.md` § Scope, `docs/policy.example.yaml`, `docs/history/1.1.0a98-*.md` + README line, the two version files.

## D26. fwd emits a one-shot handoff bundle (v1.1.0a99, constitutional amendment; phased, emit-first)

**Status:** Accepted — operator-authorized, ratified under flaresystems **ADR-0003** (interface-cohesion; clif = reference consumer; `fwd-client` = shared primitive, unchanged). A constitutional amendment to the onboarding/handoff doctrine, bundled with the current-state-doc reconciliation it requires (Core #18). **Phased cutover (emit-first, retire-after-proven):** this ship (Unit 4a) ADDS bundle emission; the `--clif-env-dir` `.env` write retirement is **deferred to Unit 4b** (separately gated, after the Songbird canary).

**Context.** ADR-0001's handoff is "a one-shot local bundle, no consumer key": fwd composes a pinned v1 JSON bundle carrying the bearer caller-token VALUES it just minted, to a local mode-0600 host file; the keyless consumer imports it (`clifctl import-credentials`, already built), writes each token into its per-network `.env.<network>`, then consumes (deletes) the bundle. Through a98 fwd minted the tokens but only ever wrote them into clif's `.env.<net>` (`install/onboard::env_write`) or printed them (`callers create`) — it did not emit the contract's bundle. This ship closes that, in lockstep with the clif consumer side.

**Decision.** A new `src/fwd/app/bundle_emit.py` composes the pinned v1 bundle (`consumer-contract-v1` §4 — `version/consumer/network/issued_at/expires_at/capabilities[]` with per-entry `capability_id/caller_token_env/caller_token/wallet_name`) and writes it mode-0600 atomically. Two emit entry points over that one helper: **(a)** `fwd onboard` composes + writes `${CLIF_ENV_DIR}/.fwd-bundle.<net>.json` from the per-network tokens it already mints (a98 made those mints capability-keyed), **alongside** the existing `env_write` (the env-write STAYS, phased); **(b)** `clifwd capability grant --emit-bundle <path> [--replace]` re-mints + re-emits for rotation (the minted token goes to the 0600 bundle instead of stdout; fail-closed — a partial mint writes no bundle). The bundle path is OUTSIDE fwd's backup/litestream scope (R3 — it is a transient consumer-side secret, never a fwd-replicated artifact). Emission is a **local file write** — no outbound connection (zero-egress intact); timestamps use fwd's local clock. **Core #7 is load-bearing and guarded:** the bundle holds BEARER tokens, never a signing key — `compose_bundle` refuses any `*PRIVATE_KEY*` env name or value (carries over the `env_write` refuse-guard) and rejects a token clif's importer would reject. The signing path / policy engine / sealed master are UNTOUCHED. **The golden test (R7) runs fwd's emitted bundle through clif's REAL `credentials.py::validate_bundle`** (the frozen acceptance oracle, imported from the co-located clif repo; skip-if-absent) — never a hand-copied fixture.

**Two custody caveats (recorded honestly, Core #18).**
- **Deviation #1 — `capability grant --emit-bundle` writes a CONTAINER-relative path.** That CLI runs in-container (it mints via `fwd:8080`, which has no host port), and `${CLIF_ENV_DIR}` is not mounted into fwd (de-intermingle, D23). So the **canary uses the `onboard` HOST-side path**; `--emit-bundle` is the rotation path, with host delivery via a bind-mount / `docker cp` (documented in its `--help`).
- **Deviation #2 — the `onboard` bundle WRITE is host-side shell, not Python `write_bundle`** — same root cause (`/opt/clif` is host-only). The in-container Python composes + Core-#7-guards (`clifwd bundle compose`, stdin tuples → stdout JSON); `onboard` writes the 0600 file mirroring `env_write` (incl. the `grep -qi PRIVATE_KEY` guard + `chown 1000`). The token transits the `docker exec` stdout capture pipe — **identical exposure to the existing `caller_ensure`** token capture.

**Bounded surface.** Code (`src/` + `install/`, implementer, accepted — NO must-fix): `app/bundle_emit.py` (new), `cli/bundle.py` (new — `bundle compose`), `cli/capability.py` (`--emit-bundle`/`--replace`/`--ttl-seconds`), `cli/main.py` (mount), `install/onboard` (additive `bundle_emit()` after `env_write`; `env_write`/`--clif-env-dir` byte-unchanged), three `tests/unit/` files (incl. the clif golden oracle). Doctrine (Reviewer): this D-record, `CLAUDE.md` § Scope + § "What FWD IS NOT", `docs/history/1.1.0a99-*.md` + README line, the two version files.

**Deferred to Unit 4b (gated, after the Songbird canary).** Retiring the `--clif-env-dir` `.env` write (the bundle becomes the sole handoff). Not claimed retired here.

## D27. Invariant #5 CLOSED — fwd never reads or writes a consumer's `.env`; the complete v2 bundle is the sole handoff (v1.1.0a101, constitutional amendment)

**Status:** Accepted — operator-authorized 2026-06-10, ratified under flaresystems **ADR-0003** (the membrane finale). A constitutional amendment closing **Invariant #5** — *fwd never reads OR writes a consumer's env* — the breach `fwd` existed to close, now closed. Bundled with the current-state-doc reconciliation it requires (Core #18). Supersedes the D26 deferral.

**Context.** Through a99, `onboard` still wrote clif's per-network `.env.<net>` (`env_write`) AND read it (token recovery + a pre-flight `SIGNING_POLICY_ADDRESS` read). That is the membrane breach: the custody daemon touching a consumer's env. The operator decided (2026-06-10): **(a) full closure** — remove both the `.env` write and the `.env` read — and **(b) the config goes IN the bundle**, not dropped, not clif-self-sourced.

**Decision.** `onboard` emits **one complete v2 bundle** (`consumer-contract-v1` §4, now **v2**): a `config` section (the non-secret network config `env_write` used to write — endpoint, identity/recipient addresses, behaviour flags, timings, `CLIF_STATE_DIR`) PLUS the per-capability bearer tokens. fwd composes it from **onboard flags + the same literal defaults** (no `.env` read); clif's v2 `import-credentials` writes clif's own `.env` from it. **fwd touches no `.env` path** — `env_write`, the token-recovery read, the pre-flight read, and the `getval` helper are all deleted; `caller_ensure` **rotates** on an existing caller (revoke + re-mint), since fwd retains no token plaintext (Core #1/#16) and may not read clif's `.env` to recover one. Core #7 extends to the config (refuse a `*PRIVATE_KEY*` config key/value; reject control-char values). The signing path / policy engine / sealed master / `caller_create` / `write_bundle` are UNTOUCHED; emission stays a local file write (no egress).

**The RPC-override delta (recorded honestly, Core #18).** `env_write` conveyed per-network RPC overrides by **reading them from clif's existing `.env`** (preserve-existing). Reading `.env` is now forbidden, and onboard has no flag for them — so fwd **cannot** emit RPC overrides in the bundle. This is acceptable: RPC overrides are **clif-side** now — the operator sets them in clif's `.env`, and clif's idempotent **upsert** import preserves any line the bundle does not set. (clif's allowlist includes the RPC keys, so a future flag-sourced RPC could be added without a `.env` read.)

**fwd ⊥ clif — the daemon never imports a consumer, even in tests (operator directive 2026-06-10).** The membrane runs one way: consumers depend on fwd's contract, never the reverse. The bundle unit test therefore pins the **`consumer-contract-v1` §4 v2** shape directly (a self-contained canonical fixture) and does **not** import clif's `validate_bundle` or reach the sibling repo. Interop with clif's real importer is proven by the **live canary**, never a unit-test code dependency.

**Bounded surface.** Code (`src/` + `install/`, implementer): `install/onboard` (delete `env_write` + its call + the recovery/pre-flight `.env` reads + `getval`; `caller_ensure` rotates; emit the v2 `config` from flags+literals), `app/bundle_emit.py` (`BUNDLE_VERSION→2`; `compose_bundle` `config` param + the Core-#7 config guard; `write_bundle`/`BundleCapability` unchanged), `cli/bundle.py` (`bundle compose` parses `CONFIG` stdin lines), the tests (the new `test_onboard_invariant5.py` behavioural closure proof + the fwd⊥clif contract-fixture rewrite of `test_bundle_emit.py`). Doctrine (Reviewer): this D-record, `CLAUDE.md` § Scope + § "What FWD IS NOT", the `install/onboard` header comment, `docs/history/1.1.0a101-*.md` + README line, the two version files.

## D28. Cold-onboard hardening — bundle config completeness (`NETWORK`), per-network manifest, reissue/reinstall (a103–a106, install/ tooling)

**Status:** Accepted — operator-driven, 2026-06-12. `install/`-only fixes (no `src/`; **no version bump** — `version.py` stays a102, see *Honest history*) closing the gaps a **cold end-to-end onboarding** surfaced after D27 relocated all config into the bundle. Does NOT relitigate D27 (the membrane holds) — it makes the relocated handoff actually complete + multi-network-safe.

**Context.** D27 moved clif's entire `.env.<net>` into the v2 bundle `config`. A cold rebuild (purge → install → onboard `--import-existing` → import → seed → up → reissue) then surfaced one hidden break per pass — each step gates the next, so each break hid the one behind it.

**Decisions.**
- **(a105) The bundle `config` MUST carry `NETWORK`.** D27's relocation composed every config key EXCEPT `NETWORK`. clif's only network selector is `net = network or os.environ.get("NETWORK") or "flare"` (clif `cli.py:715`), so a `.env.<net>` without `NETWORK` makes clif — **preflight AND the `epoch run` daemon** — silently default to **flare**: a songbird daemon would sign/claim on the wrong chain. Invisible on flare (= the default); caught only by the **songbird canary**. `install/onboard` `_be_cfg` now composes `CONFIG NETWORK <net>` first; clif accepts it because `config_env_allowlist` is derived from `Settings.network`. **General rule:** relocating a "complete config source" must **diff old vs new key sets** — a silent `or <default>` hides a dropped key; always canary on a non-default network.
- **(a106) The onboard manifest is per-network.** flare (`0x26534a`) and songbird (`0xcf3A3e`) are different on-chain identities → separate onboards. The single `config/onboard-manifest` was `mv -f`-overwritten by the 2nd onboard, breaking `fwd reissue` for the 1st. Now `config/onboard-manifest-<net>` (write loops networks; `manifest_read <net>` falls back to the legacy single file). The policy was already multi-network-safe via `clifwd policy init … --merge`. Net: multi-network onboarding is non-destructive — policy merges, manifest per-network, custody preserved.
- **(a103) `fwd reissue <net>` + outbox relocation.** Flagless, manifest-driven re-emit (rotate callers + re-publish the bundle — no wallet/key/nonce work); the outbox moved to `/opt/fwd/handoff` (outside the git clone). A plain reinstall (`install.sh` git-fetch+checkout) **preserves** `config/*` (master.key/policy/manifest), so the post-fix loop is **reinstall + reissue + re-import** — no key re-paste, no purge.
- **(a104) Imported-wallet nonces seed on import, not "INCOMPLETE".** A normal cross-membrane post-step — clif reads chain truth, `clifwd` writes fwd's nonce (fwd is zero-egress) — is expected progress, not onboard failure. `clifctl import-credentials` now auto-runs `nonce-sync` (best-effort, co-located-only).

**Honest history (Core #18).** a103–a106 shipped as `install/`-only changes on `main` with **no `version.py` bump** and **no per-version `docs/history/` entries** — a deviation from linear-forward versioning (a102, also install-only, *was* versioned). This D-record is the design record; the version/history reconciliation is a separate follow-up.

**Bounded surface.** `install/onboard` (`_be_cfg` NETWORK; per-network `manifest_write`/`manifest_read`; reissue mode), `install/fwd` (`reissue)`), `install/install.sh` (outbox bake), `.gitignore`. clif lockstep: `install/clifctl` (pre-create `.env.<net>` before import; auto-seed `_ns_one`). Contract: `consumer-contract-v1` §4 (`NETWORK` is a required `config` key).
