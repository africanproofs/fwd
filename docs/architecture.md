...
| `fwd` → sealed master | AES-256-GCM under a 32-byte mode-0600 host-file master loaded **per signing operation** and zeroized on scope exit (v1.0.0a1; no Vault, no `transit/*`, no AppRole — D1) | A compromised running `fwd`, OR host read of BOTH the master-key file AND the SQLite ciphertexts, recovers EVERY wallet plaintext key — the deliberate v1.0.0a1 trade-off for low-value automation keys on a never-public host (threat-model A4, Core inv #1). Offline theft of ciphertexts WITHOUT the master file = ciphertext only. |
...
13. fwd returns { tx_id, hash, signed_raw_tx, nonce } to caller
    — fwd does NOT broadcast (zero-egress, D20); `hash` is the tx_hash the client
...
|---|---|---|---|
| `POST` | `/v1/sign-transaction` | caller | Build → policy → sign; returns `{ tx_id, hash, signed_raw_tx, nonce }`. **Does not broadcast** (v1.1.0a9 / D20; was `/v1/sign-and-send`). |
...
Request-**body** validation failures (malformed/missing fields) use FastAPI + Pydantic's default 422 shape (`{"detail": [ {"loc": …, "msg": …, "type": …}, … ]}`), not the object form above. **There is no top-level `code` field and no `request_id`/correlation-ID field** — the earlier doc claiming a `{code, message, request_id}` envelope was aspirational; a correlation-ID handler is a Phase 10 item.
...
(An idempotency hit is NOT an error — it returns the cached `SignTransactionResponse` with HTTP 200. `rpc_failed` (502) was retired at v1.1.0a9 — `fwd` makes no RPC call; a chain-rejected broadcast is reported by the **client** via `/broadcast-result`.)
...
**There is none (v1.0.0a1).** The Vault AppRole token lifecycle that this section once described (`auth/approle/login`, lease renewal, periodic tokens — `decisions.md` D10) was retired with Vault at v1.0.0a1 (D1). The sealed master needs no authentication to anything: `SealedMaster` loads the 32-byte master key from the mode-0600 host file **on each signing operation**, decrypts in-process, and zeroizes it on scope exit (`mlockall` at lifespan startup locks the pages it briefly occupies). No token, no login, no renewal task, no Shamir unseal.
