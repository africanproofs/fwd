...
> **Read this as a historical record through v1.0.0, then the zero-egress turn (v1.1.0a8–a18).** Two architecture-level changes landed AFTER the phases below were first written and supersede mechanics described in them: (1) **Vault was retired at v1.0.0a1** — custody is now a sealed local master (`decisions.md` D1); every "Vault / unseal / Transit / AppRole" mechanic in Phases 1–7 is honest history, not current. (2) **fwd became sign-only and zero-egress at v1.1.0a9** (`decisions.md` D20) — the endpoint is `POST /v1/sign-transaction` (not `/v1/sign-and-send`), fwd does not broadcast and has no RPC client/receipt-watcher, and **clients broadcast + report back**. The current per-version narrative lives in `docs/history/` (`SHIP-LOG.md` + the per-version files); the version anchor table at the bottom of this doc is extended through v1.1.0a18.
...
> **Retired at v1.1.0a9 (zero-egress, D20):** the `receipts.py` watcher, the startup chain-reconcile (`eth_getTransactionCount` drift check), and `replacement-on-stuck` all assumed fwd reaches the chain. fwd no longer does. The nonce reservation (`BEGIN IMMEDIATE`) survives unchanged as a purely-local operation; the chain-facing pieces moved to the **client** (which broadcasts + reports back via `/broadcast-result` + `/receipt`), and replacement is now client-triggered (`/sign-replacement`, v1.1.0a13). Nonce seeding/reconciliation is via the admin `nonce-init` / `nonce-sync` endpoints, not chain probing.
...
- On-chain authorization: `ClaimSetupManager.setClaimExecutors([<new-fwd-executor>])` signed by the identity hardware wallet (`0x26534aC74153E3257dDD3471f96faA33D5D3B575` Flare); the operator's claim-recipient is unchanged. Verified on Flare Explorer.
...
| inv19 | 0.5.4 | Shipped 2026-05-17 (Reviewer-only doctrine — **Core invariant #19: forensic audit rows survive the failure they record**. Codifies the v0.5.2 drill-surfaced D16/Core-#5 defect *class* (denied/error row rolled back when the exception unwinds `session_scope`) after it recurred v0.5.0a6→v0.5.2 from an instance-patch. + #14 augmented (v0.4.5/v0.5.2 as the canonical "mocks lie" proofs + the defect-is-a-class meta-rule); D16 / architecture § Audit log / threat-model mirrored. Feedback memory written. No code change) |
| 7-gate-MET | 0.5.3 | **Shipped 2026-05-17 — Phase 7 GA verification gate MET live on Coston2** (Reviewer-only addendum). Operator funded phase7-wr-wallet → V9 (step-9 wallet-rate): call#1 mined status=0x1 from 0xa2b3…2b34 (custody recovery, block 30623108), call#2 → 403 step=9; both rows persist. Full literal 10-vector D14 matrix + approved/mined + idempotency + admin-audit proven live; `clifwd audit verify` chain-intact 25 rows exit 0. Two on-chain secp256k1 custody-recovery proofs (V10 + V9#1). Closes F2.1 + F8.2. **Phase 7 GA MET; Phase 8 (v1.0.0 ftso-fee-claimer) unblocked** (Core inv #17 dev-Vault wipe-and-redo precedes any production privkey) |
...
| audit-recon | 0.5.5 | Shipped 2026-05-17 (Reviewer-only, no Sonnet, no new code paths — audit-reconciliation. Independent stack & over-engineering audit commissioned by the operator (`docs/reviews/v0.5.4-stack-and-overengineering-audit{,-report}.md`, the 3rd independent audit); binding Reviewer pass upheld both verdicts (stack mostly-right, Vault the open operator question; over-engineering "process has become the product" — proven by #18 itself having drifted). Closed the Core #18 drift **class** (broader than the auditor cited): stray NUL byte in CLAUDE.md (file `data`, broke grep) → faithful `\u0000` text; architecture.md hot-reload→startup-only, § Backup/overview/topology/service+volume tables Scaleway-S3→v0.4.3 local-volume reality (+ deleted nonexistent `clifwd reconcile`), "Frozen for v1"→shipped-vs-deferred with the 3 unmounted endpoints Phase-marked, `transaction_args`/UE-4 honest Phase-8 marker, forensics/observability `GET /v1/audit`→`clifwd audit` CLI; config/abis/README B1-projection truth + ERC20-is-the-GA-vector (auditor "cut ERC20" CONTESTED); db.py docstring busy_timeout 5000→30000; CLAUDE.md:131 sign-message/typed-data "exist"→deferred; added the missing v0.5.4 + v0.5.5 scope paragraphs. Cuts: prometheus-client dropped (zero src use, Phase 10); coincurve KEPT+documented (eth-account secp256k1 pin — auditor "cut" CONTESTED); idempotency KEPT (GA-proven; Phase-10 marker already in D16). Operator-authority surfaced as Phase 8 entry gates. 415 passed +4 skipped unchanged; gates green; poetry.lock reconciled (prometheus-client removed); version 0.5.4→0.5.5. Terse one-line operator-authored commit, local-only. `docs/history/0.5.5-audit-reconciliation.md`) |
| audit-recon2 | 0.5.6 | Shipped 2026-05-17 (Reviewer-only HYGIENE cleanup mandated by BOTH independent audits ("one bounded cleanup, then freeze, then Phase 8"). The second independent audit `docs/reviews/v0.5.5-second-independent-audit{,-report}.md` (decision-forcing; it audited v0.5.5 AND the Reviewer's own adjudication). Verdicts converged with the first: stack mostly-right; "process has become the product". 0 Critical / 3 High / 2 Medium; the 3 Reviewer contests (ERC20 / coincurve / idempotency) independently judged SOUND. Closed the 3 High defects in the Reviewer's own v0.5.5 work, uncontested: OE-3 â NUL bytes were STILL in decisions.md / implementation-plan.md / the 0.5.5 history doc (v0.5.5 falsely claimed byte-hygiene closure; that false sentence corrected in place per Core #18), byte-stripped â \u0000 text across ALL docs (now every doc UTF-8, NUL=0); OE-3a â architecture.md fwdâVault trust-boundary still asserted the dead pre-D1 "cannot extract keys" signing property â corrected to the envelope-decrypt blast radius (threat-model A4/A5); OE-3b â dependencies.md wholesale stale (vault "signing/secp256k1", litestreamâS3, phantom `hvac`, "three services") â reconciled. No new doctrine, no new code paths. Operator-authority decisions surfaced UNCONTESTED (audit Â§6, routed past the audited Reviewer): Vault keep-vs-replace + the OE-2 flip fact (NO `vault audit enable` exists today â independently verified); the post-cleanup FREEZE of all non-Phase-8 doctrine/audit ships; local-only-history-under-GitLab-block; transaction_args-as-Phase-8-gate. version 0.5.5â0.5.6; gates green; terse local-only commit. Both audits' unanimous recommendation: this is the LAST doctrine ship before Phase 8) |
| 8a | 1.0.0a1 | Shipped 2026-05-17 (Phase 8a — Vault RETIRED, replaced by a sealed local master. Operator ruling after the asset-class reframe (~1000 FLR/key Flare automation, host never public) + both audits: `SealedMaster` (AES-256-GCM via `cryptography`, 32-byte master in a mode-0600 host file, `seal:v1:` ciphertext) replaces `VaultClient` behind the unchanged Signer seam. Sonnet-implemented (canonical prompt + wrapper, operator-gated), Reviewer binding pass + independent gate re-run + ONE Reviewer-owned correction (health.py was Vault-coupled — a gap in the Reviewer's canonical prompt, not a Sonnet failure; `/healthz` `vault` field -> `master` sealed-readiness probe; v0.4.0a5 precedent). Deleted: vault_client.py, vault-init.sh, ci_vault_init.py, config/vault/, the vault + vault-snapshot compose services + fwd-internal net + vault volumes, the CI vault stage, test_vault_client.py. Added `clifwd master generate`; `cryptography ^44.0` explicit+pinned (custody primitive; was not even transitively present). 426 passed +0 skipped (415+4skip -> the 4 ex-Vault-skip integration tests now run unconditionally); ruff/format/mypy(52)/compose green; zero functional vault refs. Doctrine reconciled IN-SHIP (freeze permits Phase-8 in-ship): Core #1/#8/#17 rewritten, Identity para, decisions.md D1 superseded (Vault history kept), architecture trust-boundary/signing-flow/Master-key section, threat-model asset-table/A3, dependencies.md. Mechanical-mirror tail (signing-flow step wording, schema comment, wallet-provisioning steps, Auth-lifecycle D10, A4/A5 deep prose, topology ASCII) phase-marked as an in-Phase-8 tail (Core #18 named deferral, NOT silent drift — closed before/with Phase 8b). version 0.5.6->1.0.0a1; terse local-only commit, operator sole author. Live Coston2 sealed-master sign gate (Core #14, mirrors v0.3.1) = operator-run addendum, does not block the code commit) |
| 8a-gate | 1.0.0a2 | Shipped 2026-05-17 (Reviewer-only — published `docs/runbooks/v1.0.0a1-sealed-master-verification.md`, the operator-run Core #14 live gate for the v1.0.0a1 custody swap: a real Coston2 tx via /v1/sign-and-send mines with on-chain `from` recovering to the fwd-sealed wallet — proving the sealed-master path end-to-end, no Vault. Mirrors v0.3.0 sign-and-send-verification / v0.5.0 phase-7-verification. No code/test change; 426 passed +0 skipped; version 1.0.0a1->1.0.0a2; terse local-only commit. The verification-met addendum is the Reviewer follow-up after the operator runs it; it gates Phase 8b) |
| 8a-gate-met | 1.0.0a3 | Shipped 2026-05-18 (Reviewer-only — **Core invariant #14 sealed-master live gate MET on Coston2**: stack recreated from the v1.0.0a1 tree (no vault/vault-snapshot), `/healthz` `master:"ok"`, wallet `sealed-verify` `0xf49C60…4231` privkey `seal:v1:`, tx `0xc222bf62…5ac5e6` mined block 30662555 `status=0x1` on-chain `from`=`$ADDR` (secp256k1 custody recovery, no Vault), `clifwd audit verify` chain intact 37 rows exit 0. Drill surfaced a runbook-shape defect (NOT code): Core #3 correctly denies empty-`data` at policy step 3 — runbook corrected in place to the proven phase-7 self-`transfer(amount=0)` shape (v0.5.1 precedent, Core #18). Verification-met addendum appended to `docs/history/1.0.0a1-vault-to-sealed-master.md`. No `src/` change beyond `version.py`; 426 passed +0 skipped; version 1.0.0a2->1.0.0a3; terse local-only commit. Phase 8b unblocked) |
| 8 | 1.0.0 | First production migration. Vault retired at v1.0.0a1 (sealed local master — D1 superseded); the v0.5.5-audit Vault-decision gate is **resolved**, the Core #14 sealed-master live gate is **MET (v1.0.0a3)**. **Remaining entry gates, in order:** (1) author + rehearse the sealed-master production-cut runbook — the Core invariant #17 dev→prod master-key provisioning (no Vault wipe-and-redo; the v1.0.0a1 #17 rewrite governs); (2) wire decoded-intent → `transaction_args` (the UE-4 deferral comes due); (3) cutover: production wallet sealed under the master, policy entry, on-chain `ClaimSetupManager.setClaimExecutors` authorization (recipient unchanged), `ftso-fee-claimer`→`clif` → `/v1/sign-and-send`, **delete the first `.env PRIVATE_KEY=` line**, one Flare epoch claimed end-to-end. Gated by Core invariant #15 (operator approval). No new generalized substrate unless this blocks on it. |
| 9a-fsp | 1.1.0a1–a7 | Shipped 2026-05-18/19 — FSP protocol-message signing: `POST /v1/sign-fsp-message` (EIP-191 over a fwd-**reconstructed** messageHash for UPTIME / REWARD_DISTRIBUTION — a1 signer primitive, a2 endpoint), + a3 constitutional ship-types (D17), a4 hand-off-embed (D18) + first live Coston2 FSP drill (GATE-1 F1), a5 bounded reconciliation, a6 key-domain segmentation carve-out, a7 admin `wallets balances`. Per-version detail in `docs/history/`. |
| 9b-zero-egress | 1.1.0a8–a18 | Shipped 2026-05-27/28 — **fwd became sign-only and zero-egress (D20)**: a8 admin `nonce-init`; a9 excision (`rpc.py`/receipt-watcher/reconcile/balances deleted, `/v1/sign-and-send`→`/v1/sign-transaction`, no broadcast); a10 client report-back (`/broadcast-result`+`/receipt`); a11 network lockdown (`internal: true`); a12 funded live Coston2 drill + checksum/nonce-release fix; a13 client-triggered `/sign-replacement` + admin `nonce-sync` + `nonce/holes` (migration 0007); a14 D20 constitutional amendment; a15 no-host-port correction; a16 public README rewrite; a17 docs-tree reconciliation; a18 external-review doc fixes (this set). Consumer `clif` migrated (v0.5.5); epoch-400 Flare/Songbird live drill passed. Per-version detail in `docs/history/`. |
| 9-rolling | 1.1.x… | Remaining `.env PRIVATE_KEY` migrations (apregister Coston2 test wallet, apcli, fics write paths) — operator-gated, Core invariant #15. |
| 10 | 2.x | Hardening (deferred) |

The version anchor lives in `src/fwd/version.py`, surfaced by `clifwd version` (NOT in `/healthz`, which returns only `master` + `fwd`). Cross-artifact drift (pyproject.toml ≠ version.py) is caught by a unit test (`tests/unit/test_version_consistency.py`) — added in Phase 2.
