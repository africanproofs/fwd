...
| 7-a6 | 0.5.0a7 | Integration REST (the split's second half): admin-action audit authorship — `AdminScope` context manager threading `AuditRepo` through `wallet-create`/`wallet-import`/`caller-create`/`caller-revoke` (api/wallets.py + api/callers.py); idempotency-replay path (Idempotency-Key → cached tx_id, `sign-and-send-duplicate` audit row, no policy re-eval, no rate increment per D14); admin-endpoint `policy_path` validation (POST /v1/admin/callers|wallets reject 400 if path not in loaded policy, per D14/v0.5.0a2); `delete_stale` policy-load-path invocation (bounded bucket growth); `audit-verify-failure` privileged self-write-on-break path (or Phase 10) |
| 7-gate | 0.5.0 | Phase 7 GA verification gate met live (after a6+a7; runbook drives operator through: provision FTSO-claimer-shaped policy + caller; attempt sanctioned action → approved; attempt sanctioned-but-modified arg → refused; attempt unsanctioned method → refused; `docker exec fwd clifwd audit verify` walks the chain cleanly afterwards. F2.1 + F8.2 audit deferrals marked closed) |
