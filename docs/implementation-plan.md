...
> **Read this as a historical record through v1.0.0, then the zero-egress turn (v1.1.0a8–a16).** Two architecture-level changes landed AFTER the phases below were first written and supersede mechanics described in them: (1) **Vault was retired at v1.0.0a1** — custody is now a sealed local master (`decisions.md` D1); every "Vault / unseal / Transit / AppRole" mechanic in Phases 1–7 is honest history, not current. (2) **fwd became sign-only and zero-egress at v1.1.0a9** (`decisions.md` D20) — the endpoint is `POST /v1/sign-transaction` (not `/v1/sign-and-send`), fwd does not broadcast and has no RPC client/receipt-watcher, and **clients broadcast + report back**. The current per-version narrative lives in `docs/history/` (`SHIP-LOG.md` + the per-version files); the version anchor table at the bottom of this doc is extended through v1.1.0a16.
...
| 9b-zero-egress | 1.1.0a8–a17 | Shipped 2026-05-27 — **fwd became sign-only and zero-egress (D20)**: a8 admin `nonce-init`; a9 excision (`rpc.py`/receipt-watcher/reconcile/balances deleted, `/v1/sign-and-send`→`/v1/sign-transaction`, no broadcast); a10 client report-back (`/broadcast-result`+`/receipt`); a11 network lockdown (`internal: true`); a12 funded live Coston2 drill + checksum/nonce-release fix; a13 client-triggered `/sign-replacement` + admin `nonce-sync` + `nonce/holes` (migration 0007); a14 D20 constitutional amendment; a15 no-host-port correction; a16 public README rewrite; a17 docs-tree reconciliation (this set). Consumer `clif` migrated (v0.5.5); epoch-400 Flare/Songbird live drill passed. Per-version detail in `docs/history/`. |
