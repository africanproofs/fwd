...
| 7-gate | 0.5.0 | Runbook published 2026-05-16 (Reviewer-only doc ship — `docs/runbooks/phase-7-verification.md`: operator-gated FULL 10-vector D14 deny matrix + ERC-20 `transfer` approved/mined + idempotency replay + admin-audit no-secret + `clifwd audit verify`; version 0.5.0a7→0.5.0; commit-attribution doctrine reconciled to the root constitution). **Live gate PENDING operator execution** — Core invariant #14: the live run is the validation; the verification-met addendum (on-chain evidence) is a Reviewer-only follow-up to `docs/history/0.5.0-phase-7-ga.md`, mirroring v0.4.1/v0.4.6. A defect the drill surfaces → one combined fix+evidence ship (v0.5.1, the v0.4.5/v0.4.6 precedent). F2.1 + F8.2 audit deferrals close on a clean live pass |
