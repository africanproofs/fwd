...
| 8 | 1.0.0 | First production migration. Vault retired at v1.0.0a1 (sealed local master — D1 superseded); the v0.5.5-audit Vault-decision gate is **resolved**, the Core #14 sealed-master live gate is **MET (v1.0.0a3)**. **Remaining entry gates, in order:** (1) author + rehearse the sealed-master production-cut runbook — the Core invariant #17 dev→prod master-key provisioning (no Vault wipe-and-redo; the v1.0.0a1 #17 rewrite governs); (2) wire decoded-intent → `transaction_args` (the UE-4 deferral comes due); (3) cutover: production wallet sealed under the master, policy entry, on-chain `setClaimRecipient` rotation, `ftso-fee-claimer` → `/v1/sign-and-send`, **delete the first `.env PRIVATE_KEY=` line**, one Flare epoch claimed end-to-end. Gated by Core invariant #15 (operator approval). No new generalized substrate unless this blocks on it. |
