...
| 7-a2 | 0.5.0a3 | ABI intent decoder substrate (pure function `decode_intent(contract, calldata, abi) -> DecodedIntent | None`; ABI registry loader; config/abis/reward_manager.json + participant_register.json + erc20.json; unit tests for happy path per contract + unknown selector + unknown contract + malformed bytes + partial decode + address normalization + string/bytes round-trip. No integration with sign-and-send yet) |
| 7-a3 | 0.5.0a4 | Policy engine substrate (`src/fwd/domain/policy.py` Pydantic schema; `src/fwd/app/policy_engine.py` evaluator implementing D14's 10 steps; `src/fwd/infra/rate_repo.py` for rate_buckets + wallet_buckets; Alembic 0005 creates both tables; startup fail-fast: orphan callers / orphan signatures / orphan wallet refs; rate-bucket release-on-failure helpers; admin-endpoint policy_path validation; unit tests for full evaluation matrix incl. default-deny per step + release-on-failure paths. No integration with sign-and-send yet) |
| 7-a4 | 0.5.0a5 | Audit log substrate (`src/fwd/infra/audit_repo.py` with canonical-JSON hash-chain writer + chain-verify function; `src/fwd/cli/audit.py` with `verify`, `show`, `tail` subcommands; canonical access via `docker exec fwd clifwd audit ...`; unit tests for chain construction + verification + chain-break detection + genesis row + canonical-JSON round-trip. No integration) |
| 7-a5 | 0.5.0a6 | Integration ship (sign-and-send rewritten to call intent decoder → policy engine → audit writer in sequence; RequestScope extended with `policy_engine` + `audit_repo`; AdminScope ships for admin-endpoint audit authorship; idempotency-replay path writes sign-and-send-duplicate audit row; existing fixtures get a test policy.yaml; synthetic-attack test that validates default-deny against curated malicious inputs — unknown method, value > max, wrong arg_predicate, beyond rate, etc.; existing callers in DB without a resolvable policy_path → startup fail-fast) |
| 7-gate | 0.5.0 | Phase 7 GA verification gate met live (runbook drives operator through: provision FTSO-claimer-shaped policy + caller; attempt sanctioned action → approved; attempt sanctioned-but-modified arg → refused; attempt unsanctioned method → refused; `docker exec fwd clifwd audit verify` walks the chain cleanly afterwards. F2.1 + F8.2 audit deferrals marked closed) |
