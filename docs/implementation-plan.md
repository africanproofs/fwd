...
| 8 | 1.0.0 | First production migration. **Entry gates (from the v0.5.5 audit, in order):** (1) OPERATOR DECISION — keep Vault vs Shamir-sealed-local-master (sharpened: a sealed master lacks only an independent decrypt-audit-trail a compromised fwd can't rewrite; Phase-0 must verify whether Vault's audit device is even enabled in `vault-init.sh`); (2) author + rehearse `docs/runbooks/vault-production-cut.md` — the Core invariant #17 dev→prod wipe-and-redo, currently doctrine prose with no runbook; (3) wire decoded-intent → `transaction_args` (the UE-4 deferral comes due); (4) cutover: production wallet in the re-initialized Vault, policy entry, on-chain `setClaimRecipient` rotation, `ftso-fee-claimer` → `/v1/sign-and-send`, **delete the first `.env PRIVATE_KEY=` line**, one Flare epoch claimed end-to-end. No new generalized substrate unless this blocks on it. |
